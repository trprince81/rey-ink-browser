...
export default async function handler(req,res){
 ...
 try{
  if(req.method==='GET'){
   ...
   const r=await db('rey_ink_licenses?select=id,license_key,client_name,client_email,status,starts_at,expires_at,device_id,plan_id,access_mode,metadata,created_at,updated_at&order=created_at.desc&limit='+limit);
  ...
  if(action==='create_owner'){
   const email=String(b.email||'').trim().toLowerCase(),name=String(b.name||'Mutant Owner').trim(),days=Math.max(1,Math.min(Number(b.days||3650),3650));
   if(!/^\S+@\S+\.\S+$/.test(email))return out(res,{ok:false,error:'Correo válido obligatorio'},400);
   ...
   const starts=new Date(),expires=new Date(starts.getTime()+days*86400000),license_key=key();
   const r=await db('rey_ink_licenses',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({license_key,client_name:name,client_email:email,status:'active',starts_at:starts.toISOString(),expires_at:expires.toISOString(),access_mode:'owner_unlimited',metadata:{product:'Mutant',role:'owner',entitlements:{unlimitedProfiles:true,unlimitedDevices:true,unlimitedSessions:true}}})});
   if(!r.ok)return out(res,{ok:false,error:await r.text()},500);return out(res,{ok:true,license:(await r.json())[0]});
  }
  if(action==='set_owner'){
   const id=String(b.id||'');if(!id)return out(res,{ok:false,error:'Licencia obligatoria'},400);
   const r=await db('rey_ink_licenses?id=eq.'+encodeURIComponent(id),{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({access_mode:'owner_unlimited',metadata:{product:'Mutant',role:'owner',entitlements:{unlimitedProfiles:true,unlimitedDevices:true,unlimitedSessions:true}},updated_at:new Date().toISOString()})});
   if(!r.ok)return out(res,{ok:false,error:await r.text()},500);return out(res,{ok:true,license:(await r.json())[0]});
  }
  if(action==='create'){
   ...
   const r=await db('rey_ink_licenses',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({license_key,plan_id:planId,client_name:name,client_email:email,status:'active',starts_at:starts.toISOString(),expires_at:expires.toISOString(),access_mode:'licensed',metadata:{product:'Mutant',role:'client'}})});if(!r.ok)return out(res,{ok:false,error:await r.text()},500);return out(res,{ok:true,license:(await r.json())[0]});
  }
  if(action==='renew'){
