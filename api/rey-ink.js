...
// Extension registers one PC slot and polls for commands.
// This transport is intentionally small; for multi-instance durable production state,
// a shared store should be added later. No Supabase is used here.
...
const json = (body,status=200)=>new Response(JSON.stringify(body),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store','access-control-allow-origin':'*','access-control-allow-methods':'POST, OPTIONS','access-control-allow-headers':'content-type, authorization'}});
...
export default async function handler(req){
  ...
  if(action==='list_devices'){
    return json({ok:true,devices:Array.from({length:20},(_,i)=>{const pc_slot=i+1,d=devices.get(pc_slot);return{pc_slot,is_online:online(d),last_seen:d?.lastSeen||null,state:d?.state||null}})});
  }
  ...
  if(action==='register_device'||action==='heartbeat'){
    ...
    const previous=devices.get(pc_slot);
    devices.set(pc_slot,{pc_slot,token:body.token,lastSeen:Date.now(),state:body.state||previous?.state||null});
  ...
  if(action==='command'){
    ...
    commands.set(pc_slot,{id:commandId,command,payload:body.payload||{},createdAt:Date.now()});
    return json({ok:true,command_id:commandId,command});
  }
  if(action==='poll_command'){
    if(!tokenOK(device,body)) return json({ok:false,error:'Token inválido'},401);
    device.lastSeen=Date.now();
    const command=commands.get(pc_slot);
    if(!command) return json({ok:true,command:null});
    commands.delete(pc_slot);
    return json({ok:true,command});
  }
  if(action==='command_result'){
    if(!tokenOK(device,body)) return json({ok:false,error:'Token inválido'},401);
    device.lastSeen=Date.now();
    results.set(pc_slot,{...body.result,command_id:body.command_id,receivedAt:Date.now()});
    return json({ok:true});
  }
