...
export default async function handler(req){
  ...
  if(a==='register_device'||a==='heartbeat'){
    ...
    const token=String(b.token||old?.token||crypto.randomUUID());
    const d={pc_slot:n,token,lastSeen:Date.now(),state:b.state||old?.state||null};
    await write('device',n,d,90);
  ...
  if(a==='poll_command'){
    ...
    d.lastSeen=Date.now();await write('device',n,d,90);
    const c=await read('command',n);if(!c)return json({ok:true,command:null});
    await remove('command',n);return json({ok:true,command:c});
  }
  if(a==='command_result'){
    if(!d||b.token!==d.token)return json({ok:false,error:'Token inválido'},401);
    d.lastSeen=Date.now();await write('device',n,d,90);
    await write('result',n,{...(b.result||{}),command_id:b.command_id,receivedAt:Date.now()},300);return json({ok:true});
  }
