...
const mem=globalThis.__reyInkDevicesV8||(globalThis.__reyInkDevicesV8={devices:new Map(),commands:new Map(),results:new Map(),commandResults:new Map()});
...
async function write(kind,slot,value,ttl=90){const v=await kv('SET',[key(kind,slot),JSON.stringify(value),'EX',ttl]);if(v===null){const bucket=mem[`${kind}s`]||(mem[`${kind}s`]=new Map());bucket.set(Number(slot),value)}}
...
const a=String(b?.action||'').trim();if(a==='health')return send(res,{ok:true,service:'rey-ink',version:'9.1',storage:storageName(),time:Date.now()});if(a==='list_devices')return send(res,{ok:true,devices:await devices(),storage:storageName()});
...
if(a==='command_result'){if(!d||b?.token!==d.token)return send(res,{ok:false,error:'Token inválido'},401);d.lastSeen=Date.now();await write('device',n,d,90);const result={...(b?.result||{}),command_id:b?.command_id,receivedAt:Date.now()};if(String(b?.command_id||'')==='__screen__')await write('result',n,result,300);else await write('commandResult',n,result,90);return send(res,{ok:true})}
