...
// Rey Ink relay API: Vercel Node serverless handler.
// IMPORTANT: this file uses Vercel's Node (req, res) signature. Returning a
// Web Response from a default Node handler causes Vercel to ignore the body
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'health' || !action) {
        return send(res, { ok: true, service: 'rey-ink', version: '8.0', storage: storageName(), time: Date.now() });
      }
      if (action === 'list_devices') {
        return send(res, { ok: true, devices: await devices(), storage: storageName() });
      }
    ...
    try {
      ...
      return send(res, { ok: false, error: 'JSON inválido' }, 400);
    }

    const action = String(body?.action || '').trim();
    if (action === 'health') return send(res, { ok: true, service: 'rey-ink', version: '8.0', storage: storageName(), time: Date.now() });
    if (action === 'list_devices') return send(res, { ok: true, devices: await devices(), storage: storageName() });

    const n = Number(body?.pc_slot);
    if (!Number.isInteger(n) || n < 1 || n > 20) return send(res, { ok: false, error: 'PC inválida' }, 400);
    ...
    if (action === 'register_device' || action === 'heartbeat') {
      ...
      return send(res, { ok: true, pc_slot: n, token, is_online: true, storage: storageName() });
    }

    const device = await read('device', n);

    if (action === 'get_state') {
      return send(res, { ok: true, pc_slot: n, is_online: online(device), state: device?.state || null, storage: storageName() });
    }

    if (action === 'command') {
      if (!online(device)) return send(res, { ok: false, error: 'PC sin conexión' }, 409);
      if (body?.token && body.token !== device.token) return send(res, { ok: false, error: 'Token inválido' }, 401);
      ...
      if (!allowed.includes(command)) return send(res, { ok: false, error: 'Comando no permitido' }, 400);
      const id = crypto.randomUUID();
      await write('command', n, { id, command, payload: body?.payload || {}, createdAt: Date.now() }, 60);
      return send(res, { ok: true, command_id: id, command });
    }

    if (action === 'poll_command') {
      if (!device || body?.token !== device.token) return send(res, { ok: false, error: 'Token inválido' }, 401);
      device.lastSeen = Date.now();
      await write('device', n, device, 90);
      const command = await read('command', n);
      if (!command) return send(res, { ok: true, command: null });
      await remove('command', n);
      return send(res, { ok: true, command });
    }
    ...
    if (action === 'last_result') return send(res, { ok: true, result: await read('result', n) });

    return send(res, { ok: false, error: 'Acción desconocida' }, 400);
  } catch (error) {
    return send(res, { ok: false, error: String(error?.message || error) }, 500);
  }
}
