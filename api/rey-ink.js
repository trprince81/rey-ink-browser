...
const action=String(body?.action||'').trim();if(action==='health')return send(res,{ok:true,service:'rey-ink',version:'8.1',storage:storageName(),time:Date.now()});if(action==='list_devices')return send(res,{ok:true,devices:await devices(),storage:storageName()});
