...
export default async function handler(req,res){
 ...
 try{
  ...
  const [name,monthly]=plans[p]; const start=new Date(); const expiry=new Date(start); expiry.setMonth(expiry.getMonth()+d);
  const key=`MUT-${crypto.randomBytes(4).toString('hex').toUpperCase()}-${crypto.randomBytes(4).toString('hex').toUpperCase()}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
  return res.status(200).json({ok:true,program:'MUTANT',client:{email:e},license:{license_key:key,starts_at:start.toISOString(),expires_at:expiry.toISOString(),duration_months:d,status:'active'},plan:{name,max_profiles:p,monthly_price:monthly,total_price:monthly*d}});
