...
function installApiProxy(){
 if(window.__RI_API_PROXY)return;
 const nativeFetch=window.fetch.bind(window);
 window.fetch=async function(input,init){
   const url=typeof input==='string'?input:(input&&input.url)||'';
   if(url.indexOf('https://rnduuuiskfuikzuepvnw.supabase.co/functions/v1/rey-ink-control-center-v2')===0){
     const target='/api/control-center-proxy'+(url.includes('?')?url.slice(url.indexOf('?')):'');
     return nativeFetch(target,Object.assign({},init||{}));
   }
   return nativeFetch(input,init);
 };
 window.__RI_API_PROXY=true;
 setTimeout(()=>{try{window.__RI_ADMIN?.refresh?.()}catch(_){}} ,80);
}
function glass(){
...
body{background-image:var(--ri-exact-image,none)!important;background-size:cover!important;background-position:center center!important;background-attachment:fixed!important}
body:before{opacity:.08!important}
.hero{background:rgba(7,9,16,.22)!important;backdrop-filter:blur(10px)!important;-webkit-backdrop-filter:blur(10px)!important}
.tabs{background:rgba(8,10,17,.28)!important;backdrop-filter:blur(9px)!important;-webkit-backdrop-filter:blur(9px)!important}
.section,.toolbar,.card,.table,.table th,.detail,.detailCard,.pair,.linkbox,#status,.history,.event{background-color:rgba(8,10,18,.20)!important;backdrop-filter:blur(9px)!important;-webkit-backdrop-filter:blur(9px)!important}
.card{background:linear-gradient(145deg,rgba(12,15,25,.26),rgba(9,11,19,.18))!important}
.table{background:rgba(8,10,18,.18)!important}
.table th{background:rgba(13,16,27,.28)!important}
.input,.select{background:rgba(8,10,17,.25)!important}
`;if(!s.parentNode)document.head.append(s);
 const bg=document.getElementById('ri-background-layer');if(bg){bg.style.setProperty('z-index','0','important');bg.style.setProperty('opacity','1','important')}
}
...
function boot(){installApiProxy();decorate();glass();persistMutantAdmin();nav();setInterval(function(){decorate();glass();persistMutantAdmin()},1500)}
